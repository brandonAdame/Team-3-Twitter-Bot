...
if(isset($_GET['id'])) {
	...
	if ($eventType == "remindMe"){
		echo "{\"events\":[";
		echo "{\"id\": \"$id\", \"twitterAccount\": \"$twitterAccount\", \"eventType\": \"$eventType\", \"nextRunTime\": \"$nextRunTime\", \"lastRunTime\":\"$lastRunTime\"";
		echo "}]}";
		$result = mysqli_query($myConnection, "DELETE FROM events WHERE idNumber=$id LIMIT 1") or die(mysqli_error($myConnection));
		$result = mysqli_query($myConnection, "DELETE FROM remindMe WHERE idNumber=$id LIMIT 1") or die(mysqli_error($myConnection));
	}else{
	...
	//	echo $tomorrow . "<br />";
		...
		//echo $now->format('Y-m-d H:i:s') . "\n";
			...
			if ($eventType == "localWeather"){
				...
				$newNextRunTime = $tomorrow->format('Y-m-d') . " 21:30:00";
			}else if ($eventType == "dailyQuote"){
				$newNextRunTime = $tomorrow->format('Y-m-d') . " 12:00:00";
			}else if ($eventType == "dailyWord"){
				$newNextRunTime = $tomorrow->format('Y-m-d') . " 14:00:00";
			}else{
